    ...
    // Variants 1–5 — purely decorative background planet, no collision
    {
      ...
      const _variants = [
        // 1: Classic single amber ring
        {
          ...
          glow: 10,
          tilt: 0.3,
          planet: ["#c8891a", "#7a420a", "#2d1503"],
        },
        // 2: Dual ring — thin inner gold + wide outer orange
        {
          ringIn: _sr * 1.25,
          ringOut: _sr * 1.42,
          ringCol: "#ffcc44",
          ...
          tilt: 0.28,
          planet: ["#c8891a", "#7a420a", "#2d1503"],
          ring2: { in: _sr * 1.55, out: _sr * 1.97, col: "#ff8800", glow: 12 },
        },
        // 3: High contrast — dark planet, bright neon rings
        {
          ringIn: _sr * 1.28,
          ringOut: _sr * 2.0,
          ringCol: "#ff7700",
          glow: 16,
          tilt: 0.32,
          planet: ["#1a0d03", "#2a1505", "#0d0602"],
        },
        // 4: Atmospheric — soft halo around planet, warm rings
        {
          ringIn: _sr * 1.35,
          ringOut: _sr * 1.8,
          ringCol: "#ffbb33",
          glow: 8,
          tilt: 0.35,
          planet: ["#d49922", "#6b3a08", "#1a0900"],
          atmo: "rgba(255,180,50,0.18)",
        },
        // 5: Icy — blue-white rings, grey-brown planet
        {
          ringIn: _sr * 1.28,
          ringOut: _sr * 1.95,
          ringCol: "#88ccff",
          glow: 10,
          tilt: 0.3,
          planet: ["#a09060", "#5a4430", "#1a1208"],
        },
      ];

      const _drawRing = (rIn, rOut, col, glowSz, tilt, startA, endA) => {
        ctx.save();
        ctx.scale(1, tilt);
        ctx.shadowColor = col;
        ctx.shadowBlur = glowSz;
        ctx.strokeStyle = col;
        ctx.lineWidth = Math.max(2, (rOut - rIn) * 0.6);
        ctx.beginPath();
        ctx.arc(0, 0, (rIn + rOut) / 2, startA, endA);
        ctx.stroke();
        ctx.shadowBlur = 0;
        ctx.restore();
      };
      ...
      for (let i = 0; i < 5; i++) {
        const v = _variants[i];
        ctx.save();
        ctx.translate(_xs[i], _cy);
        ctx.globalAlpha = 0.45;

        // Ring back half (π → 2π — behind planet)
        _drawRing(v.ringIn, v.ringOut, v.ringCol, v.glow, v.tilt, Math.PI, TAU);
        if (v.ring2)
          _drawRing(v.ring2.in, v.ring2.out, v.ring2.col, v.ring2.glow, v.tilt, Math.PI, TAU);

        // Planet body
        const _pg = ctx.createRadialGradient(-_sr * 0.15, -_sr * 0.25, _sr * 0.05, 0, 0, _sr);
        _pg.addColorStop(0, v.planet[0]);
        _pg.addColorStop(0.5, v.planet[1]);
        _pg.addColorStop(1, v.planet[2]);
        ctx.beginPath();
        ctx.arc(0, 0, _sr, 0, TAU);
        ctx.fillStyle = _pg;
        ctx.fill();

        // Atmospheric glow (variant 4)
        if (v.atmo) {
          const _ag = ctx.createRadialGradient(0, 0, _sr * 0.85, 0, 0, _sr * 1.5);
          _ag.addColorStop(0, v.atmo);
          _ag.addColorStop(1, "rgba(0,0,0,0)");
          ctx.beginPath();
          ctx.arc(0, 0, _sr * 1.5, 0, TAU);
          ctx.fillStyle = _ag;
          ctx.fill();
        }

        // Ring front half (0 → π — in front of planet)
        _drawRing(v.ringIn, v.ringOut, v.ringCol, v.glow, v.tilt, 0, Math.PI);
        if (v.ring2)
          _drawRing(v.ring2.in, v.ring2.out, v.ring2.col, v.ring2.glow, v.tilt, 0, Math.PI);

        ctx.restore();

        // Variant number + name
        ctx.fillStyle = "rgba(210,175,80,0.75)";
        ctx.font = "bold 12px monospace";
        ctx.textAlign = "center";
        ctx.fillText(`${i + 1}`, _xs[i], _cy + _sr + 18);
        ctx.fillStyle = "rgba(180,150,60,0.5)";
        ctx.font = "9px monospace";
        ctx.fillText(_names[i], _xs[i], _cy + _sr + 30);
      }
    }
    // --- END SATURN SHOWCASE ---
