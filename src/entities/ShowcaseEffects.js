...
function _buildPumiceVariant(cfg, radius) {
    const cellR   = radius * cfg.cellRF;
    const spacing = cellR * cfg.spacingF;
    const rowH    = spacing * 0.866;
    const span    = Math.ceil(radius * 2 / rowH) + 2;
    ...
    for (let row = 0; row < span; row++) {
        ...
        for (let col = 0; col < span; col++) {
            ...
            cells.push({ dx: dx0, dy: dy0, r: cellR * (0.85 + Math.random() * 0.30) });
        }
    }

    const blur = Math.round(cellR * cfg.blurF);
    const pad  = blur * 3 + 4;
    const sz   = Math.ceil((radius + pad) * 2);
    const half = sz / 2;
    const [r, g, b] = cfg.rgb;
    ...
    const oc  = Object.assign(document.createElement('canvas'), { width: sz, height: sz });
    const oct = oc.getContext('2d');
    oct.filter = `contrast(${cfg.contrast})`;
    oct.drawImage(blur_oc, 0, 0);
    return oc;
}
...
function drawPumiceShowcase() {
    ...
    ctx.fillText('PUMICE VARIANTEN', W / 2, 32);
    ctx.shadowBlur  = 0;
    ctx.fillStyle   = '#445';
    ctx.font        = '10px monospace';
    ctx.fillText('Farbe · Kontrast · Zellgröße · Abstand · Blur  —  Variante A–H wählen', W / 2, 50);
    ctx.fillText('Je mehr Grau + stärker Rand + fleckig wie Wolke', W / 2, 62);

    for (let i = 0; i < _pumiceVariants.length; i++) {
        const v   = _pumiceVariants[i];
        const col = i % COLS;
        const row = Math.floor(i / COLS);
        const cx  = cellW * col + cellW / 2;
        ...
        ctx.lineWidth   = 1;
        ctx.fillRect(cellW * col + 8, startY + cellH * row + 8, 18, 14);
        ctx.strokeRect(cellW * col + 8, startY + cellH * row + 8, 18, 14);
        ctx.fillStyle = '#aac';
        ctx.font      = 'bold 9px monospace';
        ctx.textAlign = 'left';
        ctx.fillText(v.label, cellW * col + 13, startY + cellH * row + 18);

        // Blob
        ctx.save();
        ctx.globalCompositeOperation = 'screen';
        ctx.drawImage(v.oc, cx - sz / 2, cy - sz / 2);
        ctx.restore();
        ...
        ctx.fillText(v.name, cx, cy + 62);

        ctx.fillStyle = '#556';
        ctx.font      = '9px monospace';
        ctx.fillText(`contrast:${v.contrast}  cellR:${v.cellRF}`, cx, cy + 74);
        ctx.fillText(`spacing:${v.spacingF}  blur:${v.blurF}`, cx, cy + 84);

        ctx.fillStyle = '#334';
        ctx.font      = '8px monospace';
        ctx.fillText(`rgb(${v.rgb.join(',')})`, cx, cy + 93);
...
if (typeof module !== 'undefined') module.exports = { drawMetaballShowcase, drawPumiceShowcase };
