...
const { RockPoly } = require('../../src/entities/RockPoly.js');

afterEach(() => jest.restoreAllMocks());

describe('RockPoly constructor', () => {
    test('position is set from constructor arguments', () => {
        const r = new RockPoly(150, 250);
        expect(r.x).toBe(150);
        expect(r.y).toBe(250);
    });
    ...
    test('radius is within expected range (22–54)', () => {
        for (let i = 0; i < 20; i++) {
            const r = new RockPoly(0, 0).radius;
    ...
    test('has exactly 16 vertices', () => {
        expect(new RockPoly(0, 0).verts).toHaveLength(16);
    });

    test('each vertex has angle and radius fields', () => {
        for (const v of new RockPoly(0, 0).verts) {
