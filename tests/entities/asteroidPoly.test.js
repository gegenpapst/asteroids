...
describe("AsteroidPoly constructor", () => {
  ...
  test("vertices count is between 16 and 22", () => {
    const a = new AsteroidPoly(0, 0, 0);
    expect(a.verts.length).toBeGreaterThanOrEqual(16);
    expect(a.verts.length).toBeLessThanOrEqual(22);
  });
...
describe("AsteroidPoly.split", () => {
  test.each([
    [0, 1],
    [1, 2],
  ])("size %i splits into 2 asteroids of size %i", (parent, child) => {
  ...
  test("children spawn within parent radius of parent position", () => {
    ...
    expect(Math.hypot(c1.x - 200, c1.y - 300)).toBeLessThanOrEqual(
      parent.radius,
    );
    expect(Math.hypot(c2.x - 200, c2.y - 300)).toBeLessThanOrEqual(
      parent.radius,
    );
  });
